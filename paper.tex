...
\paragraph{The Apple Neural Engine.}
In a companion paper~\cite{videla2026ane}, we show that the ANE
\emph{can} run a full transformer at 171~tok/s decode by mapping every
linear layer to \texttt{Conv2d(1$\times$1)}.  That work uses native
matrix multiplication (not EML) inside the convolution units.
The ANE's fixed-function accelerator has no standalone \texttt{exp()} or
\texttt{ln()} primitives exposed to user code---element-wise
transcendentals exist only as fused components of recognized patterns
(softmax, layer~norm).  EML's power lies in expressing \emph{all}
computation through $\exp/\ln$, but the ANE's power lies in
\emph{bypassing} transcendentals via fixed-function matrix and
convolution units.  Running EML-pure inference on the ANE remains an
open problem.
...
\bibitem{videla2026ane}
A.~Videla.
\newblock One Primitive: Running Conway's Game of Life and a
  Transformer on Apple's Neural Engine via Convolution.
