...
Qwen2.5-0.5B\nobreakdash-Instruct (494\,M parameters) at 2.5~tokens/s
on Apple~Silicon, producing correct English entirely through the EML
primitive.
...
graph), achieving a 43\% file-size reduction over~v1.  A systematic
purity audit of the entire codebase, enforced by an automated
integration test, verifies that every arithmetic operation routes
through \texttt{eml\_ops}---no raw operators bypass the EML chain.  All code, benchmarks, and reproduction commands
are provided.
\end{abstract}
...
\midrule
GGUF Q8 (ref.) & 644\,MB  & 1.8 & 2.50 & ``2+2 is equal to 4.'' \\
...
All code is in the \texttt{em2} repository.  Prerequisites: Rust
...
# Expected output:
#   Generated: "2+2 is equal to 4."
#   Time:      ~6s (2.5 tokens/s)
\end{lstlisting}
...
use no BLAS, no GPU shaders, no hardware-accelerated matrix operations.
The 2.5~tok/s throughput is $\sim$100$\times$ slower than
\texttt{llama.cpp} on the same hardware---but \texttt{llama.cpp} uses
Apple's Accelerate framework and Metal GPU kernels.  Our goal is not
speed but \emph{demonstrating that a single algebraic identity
suffices}.
...
\paragraph{Why not Metal GPU?}
A natural question is whether EML's core operation
$\mathrm{eml}(x,y) = \exp(x) - \ln(y)$ could be implemented as a
Metal compute shader, parallelizing the transcendental-heavy inner loop
across GPU cores.  This is entirely feasible: a Metal kernel computing
\texttt{exp(ln\_a[k] + ln\_b[k])} per thread would parallelize the
rate-limiting \texttt{exp()} calls across up to 1024~threads per
threadgroup (Apple M-series GPUs).  The matmul inner loop is
embarrassingly parallel across the $j$~(column) dimension, making it an
ideal GPU workload.  We chose to keep \textsc{emilio} CPU-only to
demonstrate that the EML primitive suffices \emph{without} hardware
acceleration, but a Metal backend is straightforward future work and
would likely yield a 10--50$\times$ throughput increase.
...
The path from theory to practice required three levels of optimization:
\begin{enumerate}
...
\item \textbf{Purity}: a line-by-line audit of the Rust codebase
  identified and eliminated all raw operator bypasses---\texttt{cos},
  \texttt{sin}, \texttt{powf}, direct \texttt{+=}, and unrouted
  \texttt{.exp()}/\texttt{.ln()} calls---replacing them with
  \texttt{eml\_ops} equivalents (including Euler's formula
  $e^{i\theta}$ for trigonometric functions).
\end{enumerate}
