...
\begin{table}[h]
\caption{GPU kernel EML purity audit.}
\label{tab:gpu-audit}
\centering
\small
\begin{tabular}{lll}
...
\midrule
\texttt{eml\_matmul} (v1--v4)     & Pure EML & \texttt{exp(ln\_a + ln\_b) $\times$ sign} \\
...
\texttt{eml\_add\_f32}             & Pure EML & Addition (free in EML) \\
\texttt{eml\_rms\_norm\_ln\_split} & Pure EML & Eqs.~(\ref{eq:eml-sq})--(\ref{eq:eml-normsign}) \\
\texttt{eml\_residual\_rms\_norm\_ln\_split} & Pure EML & Fused add + above \\
\texttt{eml\_silu\_mul} (legacy)   & \textbf{impure} & Direct \texttt{g * sig}; \textbf{unused} \\
\bottomrule
\end{tabular}
\end{table}

The one impure kernel (\texttt{eml\_silu\_mul}) is dead code, superseded
by the pure \texttt{eml\_silu\_mul\_ln}.  All active kernels pass the
EML purity requirement.
