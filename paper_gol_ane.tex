...
\begin{axis}[
  ...
  ymin=0, ymax=190,
  xtick=data,
  xticklabels={Initial, +Iverson\\layout, +Stepanov\\prealloc, +Dragon\\Book, +Brodie\\bounds, +GQA, +Pre-alloc\\buffers, +Fixed\\shapes, +Stateful\\KV},
...
\end{axis}
\end{tikzpicture}
\caption{Cumulative decode throughput. Book-driven host elimination:
9$\to$78~tok/s (8.7$\times$). Batched GQA: 78$\to$90. Pre-allocated
buffers reduce pack overhead to 0.4\%. Fixed shapes trade decode
speed for prefill (115~tok/s prefill). Stateful KV eliminates DMA
entirely: 171~tok/s (19$\times$ total).}
\label{fig:optbar}
\end{figure}
